...
def optimization_step(model, loss, data, labels, optimizer, opt_fp, opt_sf,
    ...
    # Calculating maximal ratio of weights per layer / total weights for lambda_decay
    weight_ratio = []
    for l in range(len(ternary_weights)):
        weight_ratio.append(ternary_weights[l].numel() / total_num_weights)
    max_weight_ratio = max(weight_ratio)
    ...
    for j in range(len(ternary_weights)):
        ...
        # Full update and quantization step including weight assignment
            ...
            lambda_decay = ((ternary_weights[j].numel() / total_num_weights) / max_weight_ratio)
...
def train_quantization(model, loss_fct, train_iterator, val_iterator, optimizer, opt_fp, opt_sf,
                       ...
                       n_epochs, stop_quantizing_flag=0):
    """
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        }, './model_quantization/saved_models/checkpoint.pt')
        ...
        if epoch > 0 and is_best:
            if not torch.cuda.device_count() > 1:
                torch.save(model.state_dict(), './model_quantization/saved_models/Ternary_best_acc.pt')
            else:
                torch.save(model.module.state_dict(), './model_quantization/saved_models/Ternary_best_acc.pt')

            torch.save(assignment_list, './model_quantization/saved_models/assignments.pt')
            torch.save(opt_sf.param_groups[0]['params'], './model_quantization/saved_models/centroids.pt')
