...
parser.add_argument('--prune', default=False, action='store_false',
                    help='By default no pruning applied')
parser.add_argument('--do-prune', dest='prune', action='store_true',
                    help='Setting --do-prune flag will cause pruning instead of ternarization')
...
def grid_search(train_loader, val_loader, model, loss_fct, lambda_max_divr, initial_c_divr):
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        if model.__class__.__name__ == 'MicroNet' or 'micronet' in str(args.model):
            ...
            if args.prune:
                t_weights = [param for name, param in model.named_parameters()
                             if ('_fc.weight' in name or '_se_expand.weight' in name or '_se_reduce.weight' in name)]
        ...
        for w, w_fp in zip(t_weights, initial_fp_weights):
            ...
            w.data, _, assignment = quantize(w_fp.data, w_p_initial, w_n_initial, lambda_max_divr,
                                             Lambda=0.0, lambda_decay=1, cuda=use_cuda, pruning=args.prune)
    ...
    if stop_quantizing_flag == 0 and args.epochs != 0:
        ...
        train_quantization(model, loss_fct, train_loader, val_loader, optimizer, opt_fp, opt_sf,
                           ...
                           pruning=args.prune, plot_flag=args.plot_ternarization, n_epochs=args.epochs,
                           stop_quantizing_flag=0)
    ...
    if args.retrain_epochs > 0:
        ...
        train_quantization(model, loss_fct, train_loader, val_loader, optimizer, opt_fp, opt_sf,
                           lambda_max_divr, best_assignment, initial_c_divr, args.resume, use_cuda, pruning=args.prune,
                           plot_flag=args.plot_ternarization, n_epochs=args.retrain_epochs, stop_quantizing_flag=1)
            ...
            params = [
                {'params': [param for name, param in model.named_parameters()
                            if not ('_project_conv' in name or '_expand_conv' in name or '_conv_head' in name or
                                    '_fc.weight' in name or '_se_expand.weight' in name
                                    or '_se_reduce.weight' in name)],
                                    'weight_decay': args.weight_decay},
                {'params': [param for name, param in model.named_parameters()
                            if ('_fc.weight' in name or '_se_expand.weight' in name or '_se_reduce.weight' in name)]},
                {'params': [param for name, param in model.named_parameters()
